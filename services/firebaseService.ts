...
export const firebaseService = {
  ...
  createUser: async (username: string, password: string) => {
    await db.ref(`users/${sanitizeKey(username)}`).set({ password });
  },
  ...
  listenForBroadcasts: (username: string, callback: (message: ChatMessage | null) => void) => {
    const lastSeenTimestamp = parseInt(localStorage.getItem(`lastSeenBroadcast_${username}`) || '0', 10);
    const ref = db.ref('global/broadcast');
    const listener = ref.on('value', (snapshot) => {
        const broadcast = snapshot.val();
        if (broadcast && broadcast.timestamp > lastSeenTimestamp) {
            callback({
            ...
            localStorage.setItem(`lastSeenBroadcast_${username}`, broadcast.timestamp);
        } else if (!broadcast) {
  ...
  getCallData: async (username: string) => {
    const snapshot = await db.ref(`calls/${sanitizeKey(username)}`).get();
    return snapshot.val();
  },
