...
interface FooterProps {
    ...
    isApiKeySelectionRequired: boolean;
    onClearApiKeyRequirement: () => void;
    mode: 'chat' | 'code';
    onToggleMode: () => void;
}
...
const Footer: React.FC<FooterProps> = (props) => {
    const { 
        ...
        isThinkingMode, onToggleThinkingMode, isApiKeySelectionRequired, onClearApiKeyRequirement,
        mode, onToggleMode
    } = props;
    ...
    const isDisabled = isLoading || isDuringCall || isMemoryConfirmationPending || isLive || isConnectingLive || isTtsSpeaking || isApiKeySelectionRequired || mode === 'code';
    ...
    return (
        <footer className="p-2 sm:p-4 border-t border-[var(--border-color)] flex-shrink-0 flex flex-col gap-2">
            ...
            {isApiKeySelectionRequired ? (
                <div className="w-full flex flex-col items-center justify-center gap-3 p-3 bg-black/30 border border-yellow-500 rounded-lg animate-fade-in">
                    <p className="text-sm text-yellow-300 font-semibold text-center">Video generation requires you to select your own API key.</p>
                    <button 
                        onClick={handleSelectKey}
                        className="px-4 py-2 bg-yellow-500 text-black font-title rounded-md hover:bg-yellow-400 transition-colors"
                    >
                        Select API Key
                    </button>
                    <a 
                ...
                <div id="input-container" className="flex items-center gap-2 sm:gap-3">
                    ...
                    <button onClick={onToggleLive} disabled={isDuringCall || isMemoryConfirmationPending || isLoading || isListening || isTtsSpeaking || mode === 'code'} title={isLive ? "End Live Conversation" : "Start Live Conversation"} className={`${footerBtnBaseClass} ${isLive || isConnectingLive ? 'bg-red-500 text-white' : 'bg-white/5 text-white'}`}>
                    ...
                    <div className="relative">
                        <button onClick={() => setMenuOpen(!isMenuOpen)} disabled={isDisabled || isLoading} className={`${footerBtnBaseClass} bg-white/5 text-gray-300 hover:text-white border border-[var(--border-color)]`}>
                        ...
                        <div className={`absolute bottom-14 sm:bottom-16 right-0 w-48 bg-[rgba(10,15,31,0.9)] backdrop-blur-md border border-[var(--border-color)] rounded-lg shadow-2xl z-20 transition-all duration-300 ease-in-out ${isMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                            <button onClick={() => { onToggleMode(); setMenuOpen(false); }} className={`w-full text-left px-4 py-2 hover:bg-white/5 transition-colors flex items-center gap-2 ${mode === 'code' ? 'text-cyan-400' : 'text-gray-300'}`}>
                                {mode === 'code' ? '💬 Chat Mode' : '💻 Code Mode'}
                            </button>
                            <button onClick={() => { onToggleThinkingMode(); setMenuOpen(false); }} className={`w-full text-left px-4 py-2 hover:bg-white/5 transition-colors flex items-center justify-between ${isThinkingMode ? 'text-cyan-400' : 'text-gray-300'}`}>
