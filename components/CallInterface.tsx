...
const CallInterface: React.FC<CallInterfaceProps> = ({ callState, callerUsername, remoteStream, onAnswer, onHangup }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[radial-gradient(circle,rgba(2,0,16,0.8)_0%,rgba(10,15,31,0.9)_100%)] backdrop-blur-2xl z-[100] flex items-center justify-center flex-col text-white transition-opacity duration-500 p-4">
      <div className="text-center border border-[var(--border-color)] p-8 sm:p-12 rounded-2xl bg-black/30 animate-[magical-glow_4s_linear_infinite] animate-scale-in">
        <p className="text-base sm:text-lg tracking-widest opacity-80 mb-2">{getStatusText()}</p>
        <h2 className="font-title text-4xl sm:text-5xl mb-8 text-transparent bg-clip-text bg-gradient-to-r from-[var(--accent-purple)] to-[var(--accent-cyan)]" style={{textShadow: '0 0 15px var(--glow-color-1)'}}>{callerUsername}</h2>
        <div className="flex gap-8 justify-center mt-10">
          {callState === CallState.INCOMING && (
            <button onClick={onAnswer} className="w-16 h-16 rounded-full flex items-center justify-center shadow-lg shadow-black/40 transition-transform duration-300 hover:scale-110 hover:shadow-xl hover:shadow-black/50 bg-green-500">
              <svg className="w-8 h-8 fill-white" viewBox="0 0 24 24"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"></path></svg>
            </button>
          )}
